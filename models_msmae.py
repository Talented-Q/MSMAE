...
class MaskedAutoencoderViT(nn.Module):
    ...
    def forward_encoder(self, x, mask_ratio, throw_ratio, mask_weights, epoch):
        ...
        if epoch > 40:#the beginning epoch of amt
            x, mask, ids_restore = self.masking_throwing(x, mask_ratio, throw_ratio, mask_weights)
        else :
        ...
        x = self.norm(x)
        cls = x[:, :1]
        cls = self.softmax(self.head(cls))
        cls = cls.squeeze(1)
        return x, cls, mask, ids_restore
    ...
    def forward_loss(self, imgs, pred, cls, gt, mask_flag, epoch):
        """
        imgs: [N, 3, H, W]
        pred: [N, L, p*p*3]
        ...
        cls_loss = self.criterion(cls, gt)
        con_loss = ((loss * mask_new).sum() / mask_new.sum())
        loss = con_loss + 0.1*cls_loss  # mean loss only on masked patches

        return loss, con_loss, cls_loss
 
    def forward(self, imgs, mask_weights, target, mask_ratio, throw_ratio, epoch):

        latent, cls, mask, ids_restore = self.forward_encoder(imgs, mask_ratio, throw_ratio, mask_weights, epoch)#cls新加
        pred = self.forward_decoder(latent, ids_restore, mask, mask_ratio, epoch)  # [N, L, p*p*3]
        loss, con_loss, cls_loss = self.forward_loss(imgs, pred, cls, target, mask, epoch)

        return loss, pred, mask, con_loss, cls_loss


def msmae_vit_base_patch16_dec512d8b(**kwargs):
...
def msmae_vit_large_patch16_dec512d8b(**kwargs):
    model = MaskedAutoencoderViT(
...
def msmae_vit_huge_patch14_dec512d8b(**kwargs):
    model = MaskedAutoencoderViT(
...
msmae_vit_base_patch16 = msmae_vit_base_patch16_dec512d8b  # decoder: 512 dim, 8 blocks
msmae_vit_large_patch16 = msmae_vit_large_patch16_dec512d8b  # decoder: 512 dim, 8 blocks
msmae_vit_huge_patch14 = msmae_vit_huge_patch14_dec512d8b  # decoder: 512 dim, 8 blocks
