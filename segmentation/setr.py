...
# from mmseg.my_utils import get_root_logger
...
class SMDMAE_BACKBONE(nn.Module):
...
class SETR(nn.Module):
    def __init__(self, num_classes, checkpoint, out_indices=(9, 14, 19, 23)):
        super(SETR, self).__init__()
        self.out_indices = out_indices
        self.num_classes = num_classes
        self.VIT = SMDMAE_BACKBONE()
        if checkpoint is not None:
            ckpt = torch.load(checkpoint, map_location='cpu')['model']
...
# model = SETR(num_classes=1, out_indices=(3, 5, 7, 11)).cpu()
